(function ($) {
	...
	$(document).ready(function() {
        $('.like-button').click(function() {
            var blogId = $(this).closest('form').attr('id').split('-')[2]; // Get the blog ID from the form ID
            ...
            $.ajax({
                ...
                success: function(response) {
                    // Update the likes count on successful response
                    $('#likes-count-' + blogId).text(response.likes_count);
                },
                error: function(xhr) {
                    console.error('Error liking the blog:', xhr);
                }
            });
        });
    });
